...
**shed** is an interactive, cross-platform terminal tool ( macOS & Windows ) that finds everything dev-related eating your disk — Homebrew/Winget, npm, Docker, node_modules, IDEs, git repos — and lets you clean it up on the spot.

![Dashboard](screenshots/dashboard.png)

## What it shows

- **Homebrew / Winget** — every formula/package with size, dependency status, and which of your projects use it
...
- **Apps** — `/Applications` (macOS) or installed Apps (Windows) sorted by size
...
- **Cache Cleanups** — one-click actions for brew/winget cleanup, npm cache clean, docker prune, and more (with impact warnings)
- **Settings** — configure git scan paths and depth levels, persisted across sessions

Everything is interactive: navigate with arrows, expand with Enter/→, and delete with confirmation. Cleanup actions show an estimated size and a warning about the impact (e.g., "Next build will be slower") before running. There is currently no "dry run" for cleanups, but you must manually confirm `y` after seeing the impact summary.
